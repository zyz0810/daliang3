<template>
  <div class="loginstyle">
    <canvas id="water-line-container"></canvas>
    <div class="login-container">
      <div class="system-container">
        <!-- <span class="icon-login"></span> -->
        <h1 class="login-title" style="margin-bottom: 0;">安徽大粮粮食交易所物流业务管理系统</h1>
      </div>
      <div class="login-input">
        <div>
          <el-form :model="ruleForm" :rules="rule" ref="ruleForm" label-position="left" label-width="0px" class="demo-ruleForm ">
            <el-form-item prop="account">
              <el-input type="text" class="user-name" prefix-icon="el-icon-s-custom" v-model="ruleForm.account" auto-complete="off" placeholder="请输入您的用户名"></el-input>
            </el-form-item>
            <el-form-item prop="checkPass">
              <el-input type="password" class="password" prefix-icon="el-icon-lock" v-model="ruleForm.checkPass" auto-complete="off" placeholder="请输入您的密码" @keyup.enter.native="handleSubmit"></el-input>
            </el-form-item>
...
<script>
import { requestLogin, getRouterRole } from "../api/api";
// import waterLine from '@/utils/myCanvas'
...
LINE.prototype = {
  ...
  controlStatus: function(context, hue) {
    var rate = this.FOCUS_POSITION / (this.z + this.FOCUS_POSITION),
      axis = {
        ...
        rate: rate,
        ratio:
          (this.FAR_LIMIT - this.z) / (this.FAR_LIMIT + this.FOCUS_POSITION),
        ripple: false
      };
        ...
        switch (this.status) {
          case 0:
            ...
            axis.y2 =
              ...
              Math.max(this.renderer.thresholdY2, this.y - this.LENGTH) * rate;
      ...
      case 1:
        switch (this.status) {
          case 0:
            ...
            axis.y2 =
              this.renderer.height / 2 -
              Math.max(this.floor, this.y - this.LENGTH) * rate;
          ...
          case 2:
            this.x +=
              (this.vy / this.renderer.aspectRatio) * (this.x < 0 ? -1 : 1);
            this.y += this.vy;
            this.vy += this.GRAVITY * this.RESISTANCE;

            var extendRate =
                (this.MAX_EXTEND_COUNT - this.extendCount) /
                this.MAX_EXTEND_COUNT,
              offsetY = (-this.LENGTH / 2) * extendRate,
              offsetX =
                (-offsetY / this.renderer.aspectRatio) * (this.x < 0 ? 1 : -1);

            if (
              (this.x + offsetX < 0 &&
                this.x + offsetX > -this.renderer.thresholdX2) ||
              (this.x + offsetX > 0 &&
                this.x + offsetX < this.renderer.thresholdX2)
            ) {
              axis.shadow = this.renderer.width / 2 + (this.x + offsetX) * rate;
              offsetY *=
                (this.renderer.thresholdX2 * (this.x + offsetX < 0 ? -1 : 1) -
                  this.x) /
                offsetX;
              offsetX =
                this.renderer.thresholdX2 * (this.x + offsetX < 0 ? -1 : 1) -
            ...
            if (
              this.x >= -this.renderer.thresholdX2 &&
              this.x <= this.renderer.thresholdX2
            ) {
              this.init();
            }
  ...
  render: function(hue) {
    ...
    if (axis.ripple) {
      ...
      context.lineTo(
        axis.x2,
        axis.y2 + this.LENGTH * axis.rate * (1 - axis.shadow)
      );
      context.stroke();
    }
  }
...
export default {
  ...
  methods: {
    ...
    setParameters() {
      var ctx = document.getElementById("water-line-container");
      ...
      this.aspectRatio =
        (this.thresholdY1 - this.thresholdY2) /
        (this.thresholdX1 - this.thresholdX2);
    },
    ...
    getPosition: function(x) {
      return {
        ...
        floor:
          this.thresholdY1 +
          ((this.thresholdY2 - this.thresholdY1) *
            (this.thresholdX1 - Math.abs(x))) /
            (this.thresholdX1 - this.thresholdX2)
      };
    },
    render: function() {
